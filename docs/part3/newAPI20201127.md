...
|    返回值     |                   修改相应状态，返回状态码                   |
| 12月7日前实现 | 进行提交请求/annotationTaskCommit时进行合并操作，目前只实现将所提交的标注修改userID修改为审核者的userID，然后将标注数据插入到reviewerr_measurements表中 |
...
```javascript
数据示例：
/annotationTaskCommit传入的measurementData
[
    {
